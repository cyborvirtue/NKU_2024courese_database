...
class Staff:
    ...
    def deleteStaff(self, sid, sname, sidcard):
        try:
            self.db.begin()  # 开始事务
            # 删除 checkin_client 表中的相关记录
            self.cursor.execute("DELETE FROM checkin_client WHERE check_in_sid=%s", (sid,))
            # 删除 checkin_team 表中的相关记录
            self.cursor.execute("DELETE FROM checkin_team WHERE check_in_sid=%s", (sid,))
            # 删除 client 表中的相关记录
            self.cursor.execute("DELETE FROM client WHERE register_sid=%s", (sid,))
            # 删除 hotelorder 表中的相关记录
            self.cursor.execute("DELETE FROM hotelorder WHERE register_sid=%s", (sid,))
            # 删除 team 表中的相关记录
            self.cursor.execute("DELETE FROM team WHERE check_in_sid=%s", (sid,))
            # 最后删除 staff 表中的员工记录
            ...
            print(e)
            QMessageBox().information(None, "提示", "没有相关员工信息或删除失败！", QMessageBox.Yes)
            return False
    ...
    def modifyStaff(self, row, column, value):
        ...
        try:
            ...
            column_name = SQL_COLUMN[column]

            # 禁止更新性别和身份证号
            if column_name in ['ssex', 'sidcard']:
                QMessageBox().information(None, "提示", "不能更新性别或身份证号！", QMessageBox.Yes)
                return False
            ...
            if column_name == 'sid':
                ...
                # 更新其他表中的外键
                tables_foreign_keys = {
                    "checkin_client": "check_in_sid",
                    "checkin_team": "check_in_sid",
                    "client": "register_sid",
                    "hotelorder": "register_sid",
                    "team": "check_in_sid"
                }
                ...
                try:
                    ...
                    # 更新其他表中的外键
                    for table, foreign_key in tables_foreign_keys.items():
                        update_query = f"UPDATE {table} SET {foreign_key} = %s WHERE {foreign_key} = %s"
                        self.cursor.execute(update_query, (new_sid, sid_selected))

                    # 更新 staff 表中的 sid
                    update_staff_query = "UPDATE staff SET sid = %s WHERE sid = %s"
                    self.cursor.execute(update_staff_query, (new_sid, sid_selected))
                ...
                # 非 sid 更新的情况，直接更新 staff 表
                update_query = f"UPDATE staff SET {column_name} = %s WHERE sid = %s"
                self.cursor.execute(update_query, (value, sid_selected))
                self.db.commit()
                return True
        except Exception as e:
            print(e)
...
#         # 禁止更新性别和身份证号
#         if column_name in ['ssex', 'sidcard']:
#             QMessageBox().information(None, "提示", "性别和身份证号不能更新！", QMessageBox.Yes)
#             return False
#
#         if column_name == 'sid':
#             # 更新 sid 时，调用存储过程
#             new_sid = value
#             self.cursor.callproc('update_staff', (new_sid, data[row]['sname'], data[row]['stime'], data[row]['susername'], data[row]['spassword'], data[row]['srole'], data[row]['sphone']))
#         else:
#             # 非 sid 更新的情况，调用存储过程
#             update_query = f"UPDATE staff SET {column_name} = %s WHERE sid = %s"
#             self.cursor.execute(update_query, (value, sid_selected))
...
class Room:
    ...
    def addRoom(self, rid, rtype, rstorey, rprice, rdesc, rpic):
        ...
        try:
            # 验证房间号是否为三位数
            if not (rid.isdigit() and len(rid) == 3):
                QMessageBox().information(None, "提示", "房间号必须为三位数！", QMessageBox.Yes)
                return False

            if rid[0] != rstorey:
                QMessageBox().information(None, "提示", "房间号的第一个数字必须与楼层相等！", QMessageBox.Yes)
                return False
            # 插入房间信息
            ...
            print(e)
            QMessageBox().information(None, "提示", "房间号已存在或其他错误！", QMessageBox.Yes)
            return False
...
class Ui_LoginWindow(object):
    def setupUi(self, MainWindow):
        MainWindow.setObjectName("NKU附属酒店")
...
"")
    ...
    def retranslateUi(self, MainWindow):
        ...
        MainWindow.setWindowTitle(_translate("MainWindow", "NKU附属酒店"))
